...
def enter_date():
    layout = [[gui.Text("Enter the date")], [gui.Input(key="Input")], [gui.Button("Continue")]]
...
def enter_info():
    ...
    if values["Infile"] == None:
        window2 = gui.Window("Error Message", [[gui.Text("You need to specify which file or directory to analyse")]], [[gui.Button("OK")]])
    ...
    if values["Week"]:
        ...
        if temp2 < 1 or temp2 > 48:
            window2 =  gui.Window("Error Message", [[gui.Text(range_error.format("Week", "Integer", 1, 48))]], [[gui.Button("OK")]])
            #To do: Return to the beginning of this function or special function to handel error
        else:
    ...
    if values["Overlap"]:
        ...
        if temp3 < 0.0 or temp3 > 2.9:
            window2 =  gui.Window("Error Message", [[gui.Text(range_error.format("Overlap", "Decimal", 0.0, 2.9))]], [[gui.Button("OK")]])
            #To do: Return to the beginning of this function or special function to handel error
        else:
    ...
    if values["Sensitivity"]:
        ...
        if temp4 < 0.25 or temp4 > 2.0:
            window2 =  gui.Window("Error Message", [[gui.Text(range_error.format("Sensitivity", "Decimal", 0.25, 2.0))]], [[gui.Button("OK")]])
            #To do: Return to the beginning of this function or special function to handel error
        else:
            command += "--sensitivity {} "
    return command
